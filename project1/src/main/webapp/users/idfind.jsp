		...
		$('#id-find-btn').click(function() {
            ...
            $.ajax({
                ...
                success : function(result) {
                    let res = result.trim();
                    if (res == 'no') {
                        $('#idfind').html('<div class="m-top-120">해당 정보로 계정이 존재하지 않습니다.</div>');
                    } else {
                        $('#idfind').html('<div class="m-top-100">검색된 아이디는<br><b>' + res + '</b><br>입니다.</div>');
                    }
                    $('#ok').show();
                }
            })
...
<body>
  <div class="m-top-40 text-center">
    ...
    <div id="ok" class="roomy-10"><input type="button" id="ok-btn" class="btn btn-default" value="확인"></div>
  </div>
</body>
</html>